...
- 프런트엔드: `apps/storefront` (`Next.js 16.2.10`)
- 백엔드: `apps/api` (`Spring Boot 4.0.7`)
- 데이터베이스: PostgreSQL
...
- 데모 모드에서는 전역 안내를 표시하고 mock 결제를 명시적으로 허용하며, production 모드에서는 mock 결제가 활성화되면 API 시작을 차단합니다.
- 비회원 주문은 전화번호 쿼리스트링 대신 20분짜리 HttpOnly 주문 조회 쿠키를 사용합니다.
- 재고 차감과 취소 복원은 원자 업데이트와 행 잠금으로 처리하고, 동일 idempotency key의 병렬 주문은 하나의 주문으로 수렴합니다.
- 장바구니 변경 요청은 직렬 처리하며 실패 상태를 화면에 알리고, 체크아웃 개인정보는 브라우저 저장소에 남기지 않습니다.
