...
export function AdminWorkspace({
  ...
  const [displayForm, setDisplayForm] = useState<UpdateAdminDisplayPayload>(initialDashboard.display);
  ...
  return (
    <main className="mx-auto flex w-full max-w-[1480px] flex-col px-6 py-8 sm:px-8 lg:px-10">
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h1 className="display mt-5 text-5xl font-semibold leading-[0.92] sm:text-6xl">
              운영 현황과 판매 흐름을 한 화면에 모았습니다.
            </h1>
            <p className="mt-5 max-w-3xl text-base leading-8 text-[rgba(237,244,239,0.72)]">
              상품 진열, 주문 상태, 메인 전시 문구를 빠르게 조정할 수 있는 관리자 MVP입니다.
              storefront와 같은 데이터베이스를 바라보므로 수정 결과가 바로 반영됩니다.
            </p>
          </div>
      ...
      <div className="admin-shell mt-6 xl:grid-cols-[0.92fr_1.08fr]">
        <section className="grid gap-6">
          <article className="admin-card rounded-[36px] p-8">
                ...
                startSavingDisplay(() => {
                  void (async () => {
                    try {
                      ...
                      setDisplayForm(nextDisplay);
                      setDashboard((current) => ({ ...current, display: nextDisplay }));
                      setDisplayMessage("메인 전시 문구를 저장했습니다.");
                    } catch (error) {
                      setDisplayError(getErrorMessage(error, "전시 문구 저장 중 문제가 발생했습니다."));
                    }
                  })();
                });
              }}
            >
              ...
              <button type="submit" disabled={isSavingDisplay} className="admin-button px-6 py-4 disabled:opacity-60">
                {isSavingDisplay ? "저장 중입니다." : "전시 문구 저장"}
              </button>
            </form>
          </article>
          ...
          <article className="admin-card rounded-[36px] p-8">
            ...
            <h2 className="display mt-4 text-3xl font-semibold">최신 운영 흐름</h2>
              ...
              {selectedProduct ? (
                ...
                >
                  <div className="grid gap-4 md:grid-cols-[120px_minmax(0,1fr)]">
                    ...
                    <div className="rounded-[30px] border border-[var(--line)] bg-white/62 p-5">
                      ...
                      <p className="mt-3 text-sm leading-7 text-[var(--ink-soft)]">
                        storefront 카드와 검색 결과에 바로 반영되는 핵심 필드만 수정합니다.
                      </p>
                    </div>
                  </div>
                  ...
                  <label className="grid gap-2">
                    ...
                    <input name="productName" value={productForm.name} onChange={(event) => setProductForm((current) => ({ ...current, name: event.target.value }))} className="admin-input px-4 py-3" />
                  </label>
                  <label className="grid gap-2">
                    <span className="text-sm font-medium">상품 요약</span>
                    <textarea name="productSummary" rows={3} value={productForm.summary} onChange={(event) => setProductForm((current) => ({ ...current, summary: event.target.value }))} className="admin-input px-4 py-3" />
                  </label>
                  <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <label className="grid gap-2">
                      <span className="text-sm font-medium">배지</span>
                      <input name="productBadge" value={productForm.badge} onChange={(event) => setProductForm((current) => ({ ...current, badge: event.target.value }))} className="admin-input px-4 py-3" />
                    </label>
                    <label className="grid gap-2">
                      <span className="text-sm font-medium">가격</span>
                      <input name="productPrice" type="number" min={0} value={productForm.price} onChange={(event) => setProductForm((current) => ({ ...current, price: Number(event.target.value) }))} className="admin-input px-4 py-3" />
                    </label>
                    <label className="grid gap-2">
                      <span className="text-sm font-medium">재고</span>
                      <input name="productStock" type="number" min={0} value={productForm.stock} onChange={(event) => setProductForm((current) => ({ ...current, stock: Number(event.target.value) }))} className="admin-input px-4 py-3" />
                    </label>
                    <label className="grid gap-2">
                      <span className="text-sm font-medium">인기 점수</span>
                      <input name="productPopularityScore" type="number" min={0} value={productForm.popularityScore} onChange={(event) => setProductForm((current) => ({ ...current, popularityScore: Number(event.target.value) }))} className="admin-input px-4 py-3" />
                    </label>
                  </div>
                  <label className="flex items-center gap-3 rounded-[24px] border border-[var(--line)] bg-white/62 px-4 py-3 text-sm text-[var(--ink-soft)]">
                    <input name="productFeatured" type="checkbox" checked={productForm.featured} onChange={(event) => setProductForm((current) => ({ ...current, featured: event.target.checked }))} />
                    메인 추천 상품으로 노출
                  </label>
